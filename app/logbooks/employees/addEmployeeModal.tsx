...
export default function AddEmployeeModal({
  ...
  const handleAddEmployee = (data: Employee) => {
    ...
    const newEmployee = {
      ...
      a2pEmpId: data.a2pEmpId,
      fatherName: data.fatherName,
      areaOfWork: data.areaOfWork,
      natureOfWork: data.natureOfWork,
    };

    // First API call to register the staff
    api
      .post('/auth/register-staff', newEmployee)
      .then((response) => {
        const registeredUser = response.data;
        toast.success('Employee registered successfully');

        // Second API call to add the user to the plant
        api
          .post(`/plants/${selectedPlant.plantId}/users`, {
            userIds: [registeredUser.id],
          })
          .then(() => {
            toast.success('Employee added to plant');
          ...
          .catch((error) => {
            console.error('Error adding user to plant:', error);
            toast.error('Failed to add employee to plant');
          });
      })
      .catch((error) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center">
      ...
      <div className="relative bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
        ...
        <form onSubmit={handleSubmit(handleAddEmployee)} className="space-y-4 text-slate-900">
